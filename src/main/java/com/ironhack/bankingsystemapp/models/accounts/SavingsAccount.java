...
// INTEREST RATE
    public void applyInterestRateSavings(){
        ...
        if(period.getYears()>0.999){
       super.setBalance(super.getBalance().add(super.getBalance().multiply(interestRate.multiply(BigDecimal.valueOf(period.getYears())))));
            // reset the lastInterestRateApplied Date , add the years!
            //setLastInterestRateApplied(LocalDate.now());
            setLastInterestRateApplied(lastInterestRateApplied.plusYears(period.getYears()));
        }
    }
